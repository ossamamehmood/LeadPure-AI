...
export function LoadingOverlay({ progress, estimatedSeconds }: LoadingOverlayProps) {
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 bg-black/95 backdrop-blur-xl z-50 flex flex-col items-center justify-center p-8 text-center"
    >
      <div className="relative w-48 h-48 mb-12">
        <div className="absolute inset-0 rounded-full border-2 border-white/5"></div>
        <div className="absolute inset-0 rounded-full border-t-2 border-white/20 animate-spin"></div>
        <div className="absolute inset-4 rounded-full border border-white/5 border-b-2 border-b-[#F502FD] animate-[spin_3s_linear_infinite_reverse]"></div>
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <RefreshCw className="w-16 h-16 gradient-text animate-pulse mb-2" />
          <span className="text-[10px] font-black text-white italic tracking-widest uppercase">Optimizing</span>
        </div>
      </div>
      
      <h2 className="text-4xl font-black text-white mb-3 uppercase italic tracking-tighter">Optimization Engine Active</h2>
      ...
      <div className="w-full max-w-xl mt-16 space-y-6">
        <div className="flex justify-between items-end mb-4">
          <div className="text-left">
            ...
            <p className="text-5xl font-black text-white font-mono tracking-tighter mt-1">{progress}%</p>
          </div>
          {estimatedSeconds !== null && (
            <div className="text-right">
              <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.3em]">Estimated Completion</p>
              <p className="text-2xl font-black text-white font-mono tracking-tighter mt-1">
                {estimatedSeconds < 60 
                  ? `${estimatedSeconds}s` 
